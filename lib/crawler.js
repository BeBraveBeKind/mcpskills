...
/**
 * Run all discovery sources and return deduplicated new repos tagged with
 * their discovery source (for source-stats feedback loop).
 *
 ...
 * @param {string[]} [sourceOrder] — optional ordering of sources, highest-yield first
 * @returns {{repos: Array<{repo: string, source: string}>, npmOnlyPackages: Array<{package: string, source: string}>}}
 */
async function discoverNewRepos(knownRepos, token, maxNew = 30, sourceOrder = null) {
  ...
  // Tag each repo with its source, in the order we want to process them.
  // If sourceOrder is provided, we use it to prioritize higher-yielding
  // sources first (Phase 4 feedback loop). Otherwise, natural order.
  const defaultOrder = ['mcp-registry', 'github-topics', 'github-keywords', 'npm', 'smithery'];
  const order = Array.isArray(sourceOrder) && sourceOrder.length > 0 ? sourceOrder : defaultOrder;
  const bySource = {
    'mcp-registry':   registryRepos.map(r => ({ repo: r, source: 'mcp-registry' })),
    'github-topics':  topicRepos.map(r => ({ repo: r, source: 'github-topics' })),
    'github-keywords': keywordRepos.map(r => ({ repo: r, source: 'github-keywords' })),
    'npm':            npmResult.repos.map(r => ({ repo: r, source: 'npm' })),
    'smithery':       smitheryRepos.map(r => ({ repo: r, source: 'smithery' })),
  };

  // Concatenate in the biased order
  const allTagged = [];
  for (const src of order) {
    if (bySource[src]) allTagged.push(...bySource[src]);
  }
  // Also append any sources not in the requested order (defensive)
  for (const src of Object.keys(bySource)) {
    if (!order.includes(src)) allTagged.push(...bySource[src]);
  }

  // Deduplicate — first occurrence wins (preserves source priority)
  const seen = new Set();
  const newRepos = [];
  for (const { repo, source } of allTagged) {
    const lower = repo.toLowerCase();
    if (seen.has(lower) || knownRepos.has(lower)) continue;
    seen.add(lower);
    newRepos.push({ repo, source });
  }
  ...
  for (const pkg of npmResult.npmOnlyPackages) {
    ...
    npmSeen.add(lower);
    newNpmPackages.push({ package: pkg, source: 'npm' });
  }
