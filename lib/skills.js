...
function detectSkill(treeData, packageJsonContent, repoData) {
  ...
  if (repoData) {
    ...
    if (nameDesc.includes('mcp') || nameDesc.includes('model context protocol') ||
        ...
        nameDesc.includes('openclaw') || nameDesc.includes('clawhub') ||
        nameDesc.includes('claw skill') || nameDesc.includes('claw-skill')) {
  ...
  if (repoData?.topics) {
    ...
    if (topics.some(t => t.includes('mcp') || t.includes('agent-skill') || t.includes('openclaw') || t.includes('clawhub'))) {
...
function scoreSkillSpecCompliance(treeData, skillMdContent, serverJsonContent, packageJsonContent) {
  ...
  if (!hasIdentity && skillMdContent) {
    // SKILL.md: check YAML frontmatter first, then markdown headings
    const fm = parseSkillMdFrontmatter(skillMdContent);
    if (fm && fm.name && (fm.description || fm.version)) {
      hasIdentity = true;
    } else {
      const hasName = /^#\s+\S/m.test(skillMdContent);
      const hasDesc = skillMdContent.length > 100;
...
async function fetchScanTargets(owner, repo, treeData, token) {
  ...
  // Select source files to scan — entry points, config, AND all src/ code files
  const srcTargets = files.filter(f => {
    const lower = f.toLowerCase();
    // Skip test/docs/examples/node_modules
    if (/^(?:test|tests|__tests__|docs?|examples?|node_modules|\.git)\//i.test(lower)) return false;
    // Entry points (always include)
    if (/^(?:src\/)?(?:index|main|server|app)\.[jt]sx?$/.test(lower)) return true;
    // Python entry
    if (/^(?:src\/)?(?:main|server|app|__main__)\.py$/.test(lower)) return true;
    // All JS/TS/PY files in src/ or lib/ (scan deeper)
    if (/^(?:src|lib)\/.*\.[jt]sx?$/.test(lower)) return true;
    if (/^(?:src|lib)\/.*\.py$/.test(lower)) return true;
    ...
    // Dockerfile (can contain network commands)
    if (lower === 'dockerfile' || lower.endsWith('/dockerfile')) return true;
    return false;
  }).slice(0, 20); // Expanded from 8 to 20 files
...
// --- OpenClaw SKILL.md YAML Frontmatter Parser ---

/**
 * Parse YAML frontmatter from SKILL.md content.
 * Format: content between opening and closing --- markers.
 *
 * Returns parsed fields or null if no frontmatter found.
 * Uses simple regex parsing — no YAML library dependency.
 */
function parseSkillMdFrontmatter(content) {
  ...
  // Parse simple YAML key-value pairs and nested blocks
  ...
  for (const line of lines) {
    ...
    if (kvMatch && indent === 0) {
      ...
      if (currentKey && Object.keys(currentNested).length > 0) {
      ...
      if (value === '' || value === '|' || value === '>') {
        // Block or nested object — collect in subsequent lines
    ...
    // Nested key or list item
    if (currentKey && indent > 0) {
      const nestedKv = line.trim().match(/^(\w[\w-]*):\s*(.*)/);
      const listItem = line.trim().match(/^-\s+(.*)/);

      if (nestedKv) {
        currentNested[nestedKv[1]] = nestedKv[2].trim().replace(/^['"]|['"]$/g, '');
      } else if (listItem) {
...
/**
 * Score security transparency based on SKILL.md YAML frontmatter.
 * Rewards skills that declare their security posture (even if sparse).
 *
 * Returns a bonus score (0-2) to add to tool_safety.
 *
 * Scoring:
 *   - Has `security` section at all: +0.5
 *   - Declares `credentials` or `permissions`: +0.5
 *   - Has `allowed-tools` list: +0.5
 *   - Has `requires` section (env/bins/files): +0.5
 */
function scoreSecurityTransparency(frontmatter) {
  if (!frontmatter) return { bonus: 0, details: {} };

  let bonus = 0;
  const details = {};

  // Security section
  if (frontmatter.security || frontmatter.Security) {
    bonus += 0.5;
    details.hasSecuritySection = true;

    const sec = frontmatter.security || frontmatter.Security;
    if (typeof sec === 'object' && (sec.credentials || sec.permissions)) {
      bonus += 0.5;
      details.declaresCredentialsOrPermissions = true;
    }
  }
  ...
  if (allowedTools) {
    bonus += 0.5;
    details.hasAllowedTools = true;
    details.allowedToolsCount = Array.isArray(allowedTools) ? allowedTools.length : 1;
  }

  // Requires section
  if (frontmatter.requires || frontmatter.Requires) {
    bonus += 0.5;
    details.hasRequires = true;
  }

  return { bonus: Math.min(bonus, 2), details };
}
