...
function detectSkill(treeData, packageJsonContent, repoData) {
  ...
  let skillType = null;
  if (hasSkillMd) skillType = 'agent-skill';
  else if (hasServerJson || indicators.includes('package.json-mcp-keyword')) skillType = 'mcp-server';
  else if (hasToolDefs) skillType = 'tool-provider';
  else if (indicators.includes('repo-metadata-mcp') || indicators.includes('github-topic-mcp')) skillType = 'mcp-server';

  const isSkill = indicators.length >= 1 && skillType !== null;
