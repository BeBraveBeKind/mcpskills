...
const LIMITS = {
  ...
  // `capture` mode: lead-capture endpoint. Low hourly cap so it can't be
  // used as a free unlimited-scan bypass via repeated email submissions.
  capture: { free: 3, window: 'hour' },
};

function rateLimitKey(ipHash, mode) {
  const now = new Date();
  // Hourly window applies to badge AND capture modes
  const dateKey = (mode === 'badge' || mode === 'capture')
...
async function checkRateLimit(ipHash, mode) {
  ...
  try {
    ...
    if (count >= limit) {
      const now = new Date();
      let resetAt;
      // Hourly reset for badge AND capture modes (both use hourly windows)
      if (mode === 'badge' || mode === 'capture') {
